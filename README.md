...
- 多数据源接入：支持多个 UDP sourceId、每设备独立分帧、不同协议项目分流解析和联合显示。
- 串口接入：支持常用波特率和自定义波特率，桌面端提供更完整的串口选择信息。
- MQTT 接入：Electron 软件端支持单主题连接 `mqtt://host:1883`，或启用 SSL/TLS 后连接 `mqtts://host:port`，不依赖浏览器 WebSocket MQTT 端口。
...
每个 MQTT 连接使用一个订阅主题。多设备、多协议并行接入统一通过 UDP 网关的 `sourceId` 路由实现。
...
网关配置采用“添加设备、独立分帧、绑定解析 JSON、选择数据集”的流程。每台设备只需填写 IP、`sourceId`、显示名称、帧头、帧尾和完整帧长度；重组超时、缓存和序号诊断位于设备高级设置中。保存后可直接生成并打开联合仪表盘。

“诊断”页面按设备显示在线状态、完整帧、残帧、无效帧、缓存、丢序和解析文件绑定情况。
...
3. 配置连接参数。
4. 多 UDP 模式下，在网关配置中为每个 `sourceId` 导入独立解析 JSON，并勾选需要显示的数据集。
5. 保存并打开联合仪表盘，然后连接网关查看实时数据与诊断状态。
...
- MQTT 使用单主题订阅；需要多设备、多协议时应切换到 UDP 网关模式。
