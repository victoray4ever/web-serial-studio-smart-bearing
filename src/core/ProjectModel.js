...
export class ProjectModel {
  ...
  _validateProject(data) {
    ...
    return {
      ...
      sourceIdMap: Array.isArray(data.sourceIdMap) ? data.sourceIdMap.map((source) => ({
        ...source,
        parserId: source.parserId || source.parser || '',
        parserFileName: source.parserFileName || ''
      })) : [],
      ...
      groups: (data.groups || []).map(g => {
        ...
        return {
          title: g.title || 'Group',
          widget: groupWidget,
          sourceId: g.sourceId ?? g.source ?? '',
          importedSourceId: g.importedSourceId || '',
          datasets: (g.datasets || []).map((d, i) => {
            ...
            return {
              ...
              importedSourceId: d.importedSourceId || ''
            };
          })
        };
      })
